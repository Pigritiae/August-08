...
function convertToRoman() {
    ...
    if (isNaN(num) || num < 1 || num > 3999) {
        romanOutput.textContent = 'Error: Type a Valid Number Between 1 to 3999.';
        return;
    }
    let result = '';
    let remaining = num;
...
function converToArabic() {
    const roman = romanInput.value.toUpperCase().trim();
    if (!isValidRoman(roman)) {
        arabicOutput.textContent = 'Error: Invalid Roman Number.';
        return;
    }
    let result = 0;
    for (let i = 0; i < roman.length; i++) {
        const current = romanValues.find(r => r.symbol ===
            roman[i] + (roman[i + 1] || ''));
        if (current) {
            result += current.value;
            i++;
        } else {
            const value = romanValues.find(r => r.symbol ===
                roman[i]).value;
            result += value;
        }
    }
...
romanInput.addEventListener('input', converToArabic);